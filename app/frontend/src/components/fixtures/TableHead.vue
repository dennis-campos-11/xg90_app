  ...
  <tr class="text-xs uppercase bg-gray-100 relative after:header-border-b dark:bg-neutral-900">
    ...
    <template v-for="field in fields">
      <template v-if="field.data_field.field_type === 'statistic'">
        ...
        <th class="font-medium px-3 h-10 min-w-24 max-w-24" :key="`field-${field.id}-stats-avg-diff`"
        ...
        <th class="font-medium px-3 h-10 min-w-22 max-w-22" :key="`field-${field.id}-stats-avg90-diff`"
      ...
      <template v-else-if="field.data_field.field_type === 'fact'">
        ...
        <th class="font-medium px-3 h-10 min-w-22 max-w-22" :key="`field-${field.id}-stats-percentage-diff`"
