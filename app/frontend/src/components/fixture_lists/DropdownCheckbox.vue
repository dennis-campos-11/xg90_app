    ...
    >
      <div class="p-3">
        <div class="relative">
          ...
          <input v-model="search" type="text" :placeholder="searchPlaceholder"
            class="block w-full p-2 ps-10 text-sm border-0 rounded-lg bg-gray-100 focus-visible:ring-violet-500 dark:bg-neutral-700 dark:placeholder-neutral-400 dark:focus-visible:ring-violet-500" />
        </div>
      </div>
      ...
      <div class="flex items-center p-3 rounded-b-lg bg-gray-100 dark:bg-white/5">
        <label class="inline-flex items-center cursor-pointer">
          ...
          <div
            class="relative w-9 h-5 bg-gray-300 rounded-full peer dark:bg-neutral-700 peer-checked:bg-violet-600 after:content-[''] after:absolute after:top-[2px] after:start-[2px] after:bg-white after:border after:border-neutral-300 peer-checked:after:border-violet-700 after:rounded-full after:h-4 after:w-4 after:transition-all peer-checked:after:translate-x-full">
          </div>
          <span class="ms-3 text-sm font-medium">{{ $t('miscellaneous.show_only_selected') }}</span>
        </label>
      </div>
    </div>
  </div>
