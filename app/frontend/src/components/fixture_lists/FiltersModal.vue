        ...
        >
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-10">
            <div v-for="team in ['home', 'away']" :key="team">
              <div class="flex mb-5">
                <span
                  class="inline-flex items-center px-3 bg-gray-200 border rounded-e-0 border-gray-200 border-e-0 rounded-s-md dark:bg-neutral-800 dark:border-neutral-800"
                >
                  {{ $t(`fixture_lists.filters.locations.${team}`) }}
                </span>
                <input
                  type="number"
                  v-model.number="localFilters[key][team].from"
                  ...
                  :max="fixtureListField.data_field.settings?.[key].max"
                  step="0.01"
                />
                <span
                ...
                <input
                  type="number"
                  v-model.number="localFilters[key][team].to"
              ...
              <!-- Range Slider -->
              <Slider
                :model-value="[localFilters[key][team].from, localFilters[key][team].to]"
                ...
                :min="fixtureListField.data_field.settings?.[key].min"
                :max="fixtureListField.data_field.settings?.[key].max"
                :step="fixtureListField.data_field.settings?.[key].step || 0.01"
                :range="true"
                show-tooltip="drag"
